...
test('buildFetchRoute registers retrieve union', () => {
  const route = buildFetchRoute({ action: 'retrieve', responseId: 'r1', findText: 'x' });
...
test('buildFetchRoute registers source_check union', () => {
  const route = buildFetchRoute({ action: 'source_check', responseId: 'r1', claims: ['c1'] });
...
test('buildFetchRoute rejects unknown action and mixed selectors', () => {
  assert.throws(() => buildFetchRoute({ action: 'read', responseId: 'r' }), /action must be one of/);
  assert.throws(() => buildFetchRoute({ url: 'https://example.com', urls: ['https://example.com'] }), /exactly one of/);
});
