...
test('passes through allowlisted PATH and HOME', () => {
  ...
  assert.equal(env.PATH, [parentEnv.PATH, join('/home/testuser', '.local', 'bin')].join(delimiter));
...
test('appends user tool bins after existing PATH without changing precedence', () => {
  assert.equal(
    ...
    ['/usr/bin:/bin', '/custom/uv-bin', '/custom/xdg-bin', join('/Users/test', '.local', 'bin')].join(delimiter),
  );
});
...
test('excludes vars matching BLOCKED_PATTERN (TOKEN/KEY/SECRET/AUTH/...)', () => {
  ...
  assert.equal(env.PATH, ['/usr/bin', join('/home/user', '.local', 'bin')].join(delimiter));
...
test('sidecar-manager start passes buildPythonChildEnvironment as env option', async () => {
  ...
  try {
    ...
    const { SidecarManager } = await import('../../src/sidecar/sidecar-manager.js');
