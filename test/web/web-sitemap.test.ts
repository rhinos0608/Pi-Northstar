...
test('sitemap registry: tavily configured check, none configured throws', async () => {
  const { SITEMAP_PROVIDERS, resolveSitemapProvider, tavilySitemapProvider } = await import('../../src/web/web-sitemap.js');
...
test('tavily map exact request: endpoint, method, headers, safe fixed body', async () => {
  const { mapTavilySite, TAVILY_MAP_ENDPOINT } = await import('../../src/web/web-sitemap.js');
...
test('tavily map sends validated maxPages as limit', async () => {
  const { mapTavilySite } = await import('../../src/web/web-sitemap.js');
  let body: unknown;
...
test('runSitemap preserves provider order without query', async () => {
  const { runSitemap } = await import('../../src/web/web-sitemap.js');
