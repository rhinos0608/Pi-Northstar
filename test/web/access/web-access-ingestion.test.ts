...
describe('web-access ingestion: content store (TTL/LRU/caps)', () => {
  it('stores and retrieves entries, expires after TTL, evicts LRU, enforces caps', async () => {
    const { createWebAccessContentStore } = await import('../../../src/web/access/web-access-content-store.js');
    const { WEB_ACCESS_STORE_TTL_MS } = await import('../../../src/web/access/web-access-contract.js');
    let now = 1_000_000;
    const store = createWebAccessContentStore({ now: () => now });
    assert.equal(store.size(), 0);
    const { buildWebAccessStoredEntry } = await import('../../../src/web/access/web-access-content-store.js');
  ...
  it('rejects a single entry larger than the byte cap', async () => {
    const { createWebAccessContentStore, buildWebAccessStoredEntry } = await import('../../../src/web/access/web-access-content-store.js');
    const store = createWebAccessContentStore({ now: () => Date.now() });
    const { WEB_ACCESS_STORE_MAX_BYTES } = await import('../../../src/web/access/web-access-contract.js');
...
describe('web-access ingestion: content find (selectors/slices/find)', () => {
  it('slices stored content by offset/limit and discards them when findText present', async () => {
    const { createWebAccessContentStore, buildWebAccessStoredEntry } = await import('../../../src/web/access/web-access-content-store.js');
    const { getWebAccessContent } = await import('../../../src/web/access/web-access-content-find.js');
    const store = createWebAccessContentStore({ now: () => Date.now() });
    const entry = buildWebAccessStoredEntry({
      queries: ['alpha query'],
      results: [{
...
describe('web-access ingestion: source check (artifacts/hashes/heuristic)', () => {
  it('builds sources, passages with ids+hashes, and heuristic assessments with 0.85 cap', async () => {
    const { buildWebAccessSourceCheck } = await import('../../../src/web/access/web-access-source-check.js');
