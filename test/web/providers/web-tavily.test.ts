...
import { TAVILY_SEARCH_ENDPOINT, tavilySearchAdapter } from '../../../src/web/providers/web-tavily.js';
...
async function runStream(body: Array<string | Uint8Array>, env?: Record<string, string | undefined>): Promise<{ text: string; sources: Array<{ url: string; title: string }> }> {
  const { runTavilyResearch } = await import('../../../src/web/providers/web-tavily.js');
...
test('tavily research model defaults to pro on absent/blank, accepts mini|pro|auto', async () => {
  const { resolveTavilyResearchModel, DEFAULT_TAVILY_RESEARCH_MODEL } = await import('../../../src/web/providers/web-tavily.js');
...
test('tavily research exact request: endpoint, method, headers, stream:true with default model', async () => {
  const { TAVILY_RESEARCH_ENDPOINT } = await import('../../../src/web/providers/web-tavily.js');
...
test('tavily research stream exact-size report passes, oversized terminally rejects', async () => {
  const { TAVILY_RESEARCH_TEXT_MAX_CHARS } = await import('../../../src/web/providers/web-tavily.js');
...
test('tavily research stream transport bound rejects oversized streams', async () => {
  const { runTavilyResearch, TAVILY_RESEARCH_STREAM_MAX_BYTES } = await import('../../../src/web/providers/web-tavily.js');
