...
test('video tool infers youtube from a canonical watch URL', async () => {
  await withFetch(async (input) => {
    ...
    if (url.startsWith('https://www.youtube.com/oembed')) {
      return jsonResponse(JSON.stringify({ title: 'Inferred', author_name: 'a' }));
    }
    throw new Error(`unexpected fetch ${url}`);
  }, async () => {
    const result = await callNativeTool('video', { url: 'https://www.youtube.com/watch?v=inf1', action: 'details' }, { env: {} });
...
test('media legacy video spelling rejects unsupported_action before dispatch', async () => {
  await assert.rejects(
    () => callNativeTool('media', { platform: 'youtube', action: 'video', id: 'x' }, { env: { YOUTUBE_API_KEY: 'k' } }),
    (err: unknown) => err instanceof SocialError && err.code === 'unsupported_action',
  );
});
...
test('youtube search cursor round-trips pageToken with backend pinning', async () => {
  const seen: string[] = [];
  const page = (items: unknown[], next?: string) => jsonResponse(JSON.stringify({
    items,
  ...
  await withFetch(async (input) => {
    ...
    const first = await callNativeTool('video', { platform: 'youtube', action: 'search', query: 'cats' }, { env: { YOUTUBE_API_KEY: 'k' } });
    const pagination = (first.details as { pagination?: { hasMore?: boolean; nextCursor?: string } }).pagination;
  ...
  await withFetch(async (input) => {
    ...
    const second = await callNativeTool(
      'video',
      { platform: 'youtube', action: 'search', query: 'cats', cursor: nextCursor },
      { env: { YOUTUBE_API_KEY: 'k' } },
    );
    assert.equal((second.details as { backend?: string }).backend, 'youtube-data-api');
  });
  assert.ok(seen.some((url) => url.includes('pageToken=TOKEN2')), 'second page must send the pinned pageToken');
});

test('youtube search cursor with changed selectors rejects cursor_invalid', async () => {
  let nextCursor = '';
  await withFetch(async () => {
    return jsonResponse(JSON.stringify({ items: [{ id: { videoId: 'v1' }, snippet: { title: 't' } }], nextPageToken: 'T' }));
  }, async () => {
    const first = await callNativeTool('video', { platform: 'youtube', action: 'search', query: 'cats' }, { env: { YOUTUBE_API_KEY: 'k' } });
    nextCursor = (first.details as { pagination?: { nextCursor?: string } }).pagination?.nextCursor as string;
  });
  await assert.rejects(
    () => callNativeTool('video', { platform: 'youtube', action: 'search', query: 'dogs', cursor: nextCursor }, { env: { YOUTUBE_API_KEY: 'k' } }),
    (err: unknown) => err instanceof SocialError && err.code === 'cursor_invalid',
  );
});

test('bilibili and rss report unsupported pagination (cursor rejected)', async () => {
  await assert.rejects(
    () => callNativeTool('video', { platform: 'bilibili', action: 'hot', cursor: 'bogus' }, { env: { PATH: '/nonexistent' } }),
    (err: unknown) => err instanceof SocialError && err.code === 'cursor_invalid',
  );
  await withFetch(async () => {
...
test('youtube search empty items are valid-empty and stop without extra attempts', async () => {
  ...
  await withFetch(async () => {
    ...
    const result = await callNativeTool('video', { platform: 'youtube', action: 'search', query: 'nothing' }, { env: { YOUTUBE_API_KEY: 'k' } });
    const details = result.details as { pagination?: { returned?: number; hasMore?: boolean } };
...
test('youtube entity ids are namespaced and northstar envelope validates', async () => {
  await withFetch(async () => {
    return jsonResponse(JSON.stringify({
      items: [{ id: 'v9', snippet: { title: 'Namespaced' }, contentDetails: { duration: 'PT10S' }, statistics: { viewCount: '7' } }],
    }));
  }, async () => {
    const result = await callNativeTool('video', { platform: 'youtube', action: 'details', id: 'v9' }, { env: { YOUTUBE_API_KEY: 'k' } });
    const details = result.details as {
      items?: Array<{ id?: string; backend?: string }>;
      northstar?: { data?: { kind?: string; entities?: Array<{ kind?: string }> }; errors?: unknown[] };
    };
...
test('media feed action with a youtube url still routes to rss (feed check beats url inference)', async () => {
  await withFetch(async () => {
    ...
    const result = await callNativeTool('media', { action: 'feed', url: 'https://www.youtube.com/watch?v=feedtest' }, { env: {}, lookup: offlineLookup });
...
test('youtube transcript malformed non-XML timedtext surfaces a youtube-transcript malformed_upstream error', async () => {
  const watchHtml = '<html><body><script>var ytInitialPlayerResponse = {"captions":{"playerCaptionsTracklistRenderer":{"captionTracks":[{"baseUrl":"https://www.googlevideo.com/api/timedtext?v=abc123&lang=en","languageCode":"en"}]}}};</script></body></html>';
  await withFetch(async (input) => {
    const url = String(input);
    ...
    if (url.startsWith('https://www.googlevideo.com/')) {
      return new Response('this is neither xml nor json{{{', { status: 200, headers: { 'content-type': 'text/plain' } });
    }
    throw new Error(`unexpected fetch ${url}`);
  }, async () => {
    await assert.rejects(
      () => callNativeTool('video', { platform: 'youtube', action: 'transcript', id: 'abc123' }, { env: {} }),
      (err: unknown) => err instanceof SocialError && /timedtext payload is neither valid XML nor valid JSON/.test(err.message),
    );
  });
});

test('media limit clamps with a warning', async () => {
  await withFetch(async (input) => {
    assert.match(String(input), /maxResults=50/);
    return jsonResponse(JSON.stringify({ items: [] }));
  }, async () => {
    const result = await callNativeTool('video', { platform: 'youtube', action: 'search', query: 'cats', limit: 500 }, { env: { YOUTUBE_API_KEY: 'k' } });
    assert.match(JSON.stringify((result.details as { warnings?: string[] }).warnings), /clamped to 50/);
  });
});
