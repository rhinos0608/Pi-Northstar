...
import { SOCIAL_PLATFORMS } from '../src/social-contract.js';
import { SOCIAL_WRITE_ACTIONS } from '../src/social-write-contract.js';
import {
...
} from '../src/social-write-policy.js';
...
test('no network/exec imports in boundary files', () => {
  const contractSrc = readFileSync(new URL('../src/social-write-contract.ts', import.meta.url), 'utf8');
  const policySrc = readFileSync(new URL('../src/social-write-policy.ts', import.meta.url), 'utf8');
  const banned = [
    'node:child_process',
    'node:net',
    'node:http',
