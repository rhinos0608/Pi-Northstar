...
import { capableProvidersFor, selectAutoProviders } from '../src/knowledge-domain.js';
...
test('explicit mismatch yields unsupported_option partition while capable runs', async () => {
  const { planExplicitProviders, runKgFanout } = await import('../src/knowledge-domain.js');
...
test('explicit fanout never expands beyond requested providers and respects cap', async () => {
  const { planExplicitProviders } = await import('../src/knowledge-domain.js');
...
test('auto fallback advances only on recoverable error codes, stops on success', async () => {
  const { runKgAuto } = await import('../src/knowledge-domain.js');
  const calls: string[] = [];
...
test('explicit fanout never issues cursor; cursor with explicit providers rejected', async () => {
  const { issueKgCursor, rejectCursorForExplicitFanout } = await import('../src/knowledge-domain.js');
  assert.equal(
    issueKgCursor({
      provider: 'diffbot',
      fingerprint: 'fp',
...
test('isRecoverableKgOutcome treats contract/semantic failures as failoverable without paid retry', async () => {
  const { isRecoverableKgOutcome } = await import('../src/knowledge-domain.js');
...
test('cursor with empty explicit allowlist is rejected, omitted providers allow cursor', async () => {
  const { rejectCursorForExplicitFanout } = await import('../src/knowledge-domain.js');
  try {
