...
export const SOCIAL_SEARCH_COMMAND = 'social.search';

const SEARCH_ACTION = 'search' as const;
...
interface SocialSearchArgs {
  platform: SocialPlatform;
  action: typeof SEARCH_ACTION;
 ...
 */
function parseArgs(args: Record<string, unknown>): SocialSearchArgs {
  for (const key of Object.keys(args)) {
    if (!ALLOWED_FIELDS.has(key)) {
      throw fail('invalid_request', `unknown social.search field: ${key.slice(0, 32)}`);
    }
  }
  const rawAction = args.action;
  let action: typeof SEARCH_ACTION;
  if (rawAction === undefined) {
    action = SEARCH_ACTION;
  } else if (typeof rawAction !== 'string' || rawAction.trim().length === 0) {
    throw fail('invalid_request', 'action must be a non-empty string when provided');
  } else if (rawAction.trim() !== SEARCH_ACTION) {
    // Canonical-only: legacy spellings (read/post/subreddit/note/topic/...)
    // and every other action reject here — never dispatched.
    throw fail('unsupported_action', 'social.search serves the canonical search action only');
  } else {
    action = SEARCH_ACTION;
  }
...
export function mapSocialSearchCommandResult(
  ...
  const mapped: NorthstarCommandResultV1 = {
    ...
    version: 1,
    commandId: SOCIAL_SEARCH_COMMAND,
    ...
    resolvedSurface: SOCIAL_SEARCH_COMMAND,
    attemptedSurfaces: [context.surface, SOCIAL_SEARCH_COMMAND],
...
function attachFailure(error: unknown, context: CommandContext, source: string): never {
  ...
  const mapped: NorthstarCommandResultV1 = {
    ...
    error: {
      code,
      message: error instanceof Error ? error.message : 'Social search request failed',
      retryable,
  ...
  const check = validateCommandResult(mapped);
  if (!check.ok) throw new TypeError(`Invalid mapped command result: ${check.issues.join('; ')}`);
  const target = error instanceof Error ? error : new Error('Social search request failed');
...
export function setSocialSearchExecutor(next: SocialExecutor | undefined): void {
  if (next !== undefined) scopedExecutor.enterWith(next);
  else scopedExecutor.disable();
}

export async function executeSocialSearch(
  args: Record<string, unknown>,
  context: CommandContext,
  options?: SocialSearchOptions,
): Promise<BackendCallResult> {
  let parsed: SocialSearchArgs;
  try {
  ...
  try {
    ...
    const northstarCommand = mapSocialSearchCommandResult(envelope, context);
    return {
      ...result,
      details: {
...
export const socialSearchHandler = {
  commandId: SOCIAL_SEARCH_COMMAND,
  execute: executeSocialSearch,
};
