...
function parseArgs(args: Record<string, unknown>): SocialSearchArgs {
  for (const key of Object.keys(args)) {
    if (!ALLOWED_FIELDS.has(key)) {
      throw commandFailure('invalid_request', `unknown social.search field: ${key.slice(0, 32)}`);
    }
  }
  ...
  if (rawAction === undefined) {
    ...
    throw commandFailure('unsupported_action', 'social.search serves the canonical search action only');
  } else {
    action = SEARCH_ACTION;
  }
...
function attachFailure(error: unknown, context: CommandContext, source: string): never {
  return attachExternalCommandFailure(error, context, {
    ...
    fallbackSource: 'social',
    source,
    failureMessage: 'Social search request failed',
