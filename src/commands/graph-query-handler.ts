...
export const GRAPH_QUERY_COMMAND = 'graph.query';

/** Retryable graph query failures: rate limit + upstream only. Auth/input/cursor stay terminal. */
...
export function mapGraphQueryCommandResult(envelope: GraphResult, context: CommandContext): NorthstarCommandResultV1 {
  ...
  const mapped: NorthstarCommandResultV1 = {
    schema: 'northstar.command-result.v1', version: 1, commandId: GRAPH_QUERY_COMMAND,
    ...
    requestedSurface: context.surface, resolvedSurface: GRAPH_QUERY_COMMAND,
    attemptedSurfaces: [context.surface, GRAPH_QUERY_COMMAND],
...
function attachFailure(error: unknown, context: CommandContext): never {
  ...
  const mapped: NorthstarCommandResultV1 = {
    ...
    attemptedSurfaces: [context.surface, GRAPH_QUERY_COMMAND],
    sideEffect: { started: false, settled: true, outcome: 'not_started' }, verifiedArtifacts: [], nextActions: [],
    error: { code, message: error instanceof Error ? error.message : 'Graph query request failed', retryable: retryable(code), category: code === 'cancelled' ? 'cancelled' : 'graph' },
  };
  const target = error instanceof Error ? error : new Error('Graph query request failed');
  Object.defineProperty(target, 'commandResult', { value: mapped, enumerable: false });
  throw target;
}

/**
 * Caller-neutral graph.query read over the live graph seam (callGraphTool:
 * strict request validation, cursor binding, Diffbot DQL/SPARQL dispatch).
 * Reads only: query/probe carry no mutation verbs. Limit/pageSize bounds
 * reject instead of clamping in the owning contract; this handler passes them
 * through untouched. Validation failures return as failed envelopes (never
 * thrown); only abort/transport throws via attachFailure.
 */
export async function executeGraphQuery(args: Record<string, unknown>, context: CommandContext): Promise<BackendCallResult> {
  try {
    if (context.signal?.aborted) throw Object.assign(new Error('aborted'), { name: 'AbortError' });
    if (args.action !== undefined && args.action !== 'query') {
      return attachFailure(Object.assign(new Error('action must be "query" for graph.query'), { code: 'invalid_input' }), context);
    }
    const { action: _ignored, ...rest } = args;
    const result = await callGraphTool({ action: 'query', ...rest }, {
      env: context.env,
      ...(context.signal ? { signal: context.signal } : {}),
    });
    const envelope = graphEnvelope(result);
    if (!envelope) return attachFailure(new Error('Graph query returned no envelope'), context);
    const northstarCommand = mapGraphQueryCommandResult(envelope, context);
...
export const graphQueryHandler = { commandId: GRAPH_QUERY_COMMAND, execute: executeGraphQuery };
