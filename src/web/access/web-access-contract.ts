...
export const WEB_ACCESS_FETCH_TOP_K_MAX = 20;
export const WEB_ACCESS_FETCH_MAX_PAGES_MAX = 25;
export const WEB_ACCESS_FETCH_MAX_CHARS_MAX = 50_000;
 ...
 */
const WEB_ACCESS_STATUS_KIND_BY_CODE: ReadonlyMap<number, WebAccessErrorKind> = new Map([
  [401, 'auth'],
  [403, 'auth'],
  [404, 'not_found'],
  [408, 'timeout'],
...
export function webAccessErrorKindForStatus(status: number | undefined): WebAccessErrorKind {
  const exact = status === undefined ? undefined : WEB_ACCESS_STATUS_KIND_BY_CODE.get(status);
  if (exact !== undefined) return exact;
  if (status !== undefined && status >= 300) return 'upstream_error';
  if (status !== undefined && status >= 200) return 'invalid_response';
...
function rejectOutOfRangeQueries(out: string[]): void {
...
function collectQueriesArray(value: unknown): string[] {
  if (!Array.isArray(value)) throw new WebAccessContractError('queries must be an array of strings');
  const out: string[] = [];
...
function parseQueries(raw: WebAccessRawSearchRequest): string[] {
  ...
  const out: string[] = [];
  if (hasQueries) {
    out.push(...collectQueriesArray(raw.queries));
  } else if (hasQuery) {
...
function parseDomains(value: unknown): string[] | undefined {
  ...
  const out: string[] = [];
  for (const entry of value) {
    out.push(cleanDomainHost(entry));
  }
  return out;
}

function cleanDomainHost(entry: unknown): string {
 ...
 */
function rejectOperatorSearchFields(raw: WebAccessRawSearchRequest): void {
...
function rejectLegacySearchFields(raw: WebAccessRawSearchRequest): void {
  rejectOperatorSearchFields(raw);
...
function parseIncludeContent(raw: WebAccessRawSearchRequest): boolean {
  if (raw.includeContent === undefined) return false;
  if (typeof raw.includeContent !== 'boolean') {
    throw new WebAccessContractError('includeContent must be a boolean');
  }
  return raw.includeContent;
}

function parseRecency(raw: WebAccessRawSearchRequest): WebAccessRecency | undefined {
  if (raw.recency === undefined) return undefined;
  ...
  return raw.recency as WebAccessRecency;
}

function parseKnowledge(raw: WebAccessRawSearchRequest): boolean | undefined {
  if (raw.knowledge === undefined) return undefined;
  if (typeof raw.knowledge !== 'boolean') throw new WebAccessContractError('knowledge must be a boolean');
  return raw.knowledge;
}

function parseSearchCursor(raw: WebAccessRawSearchRequest, queries: string[]): string | undefined {
  if (raw.cursor === undefined) return undefined;
...
function parseAbortSignal(raw: WebAccessRawSearchRequest): AbortSignal | undefined {
  if (raw.signal === undefined) return undefined;
  ...
  return raw.signal as AbortSignal;
}

function assembleSearchRequest(parts: WebAccessSearchRequest): WebAccessSearchRequest {
  const request: WebAccessSearchRequest = { queries: parts.queries, limit: parts.limit, includeContent: parts.includeContent };
  if (parts.recency !== undefined) request.recency = parts.recency;
  if (parts.domains !== undefined) request.domains = parts.domains;
  if (parts.category !== undefined) request.category = parts.category;
  if (parts.source !== undefined) request.source = parts.source;
  if (parts.yearFrom !== undefined) request.yearFrom = parts.yearFrom;
  if (parts.knowledge !== undefined) request.knowledge = parts.knowledge;
  if (parts.mode !== undefined) request.mode = parts.mode;
  if (parts.cursor !== undefined) request.cursor = parts.cursor;
  if (parts.signal !== undefined) request.signal = parts.signal;
  return request;
}

export function normalizeWebAccessSearchRequest(raw: WebAccessRawSearchRequest): WebAccessSearchRequest {
  rejectLegacySearchFields(raw);
  const queries = parseQueries(raw);
  const limit = parseLimit(raw);
  const includeContent = parseIncludeContent(raw);
  const recency = parseRecency(raw);
  ...
  const cursor = parseSearchCursor(raw, queries);
  const signal = parseAbortSignal(raw);
  return assembleSearchRequest({ queries, limit, includeContent, recency, domains, category, source, yearFrom, knowledge, mode, cursor, signal });
}
...
function validateProviderHit(entry: unknown, index: number, issues: string[]): number {
...
function cleanClaimEntry(entry: unknown): string {
...
function validateProviderResultBounds(results: unknown[], issues: string[]): void {
  if (results.length > WEB_ACCESS_MAX_NUM_RESULTS) {
    issues.push(`results exceed maximum of ${WEB_ACCESS_MAX_NUM_RESULTS}`);
  }
  let bytes = 0;
  for (const [index, entry] of results.entries()) {
    bytes += validateProviderHit(entry, index, issues);
  }
...
function validateProviderAnswerFields(record: Record<string, unknown>, issues: string[]): void {
...
export function validateWebAccessProviderResponse(value: unknown): { ok: boolean; issues: string[] } {
  ...
  if (!Array.isArray(record.results)) {
    ...
    validateProviderResultBounds(record.results, issues);
  }
  validateProviderAnswerFields(record, issues);
...
// claims[1..20] + sourceIds?; cached corpus only, no network.

export interface WebAccessFetchCrawlUrlSource {
  type: 'url';
  url: string;
  followLinks?: boolean | undefined;
}
export interface WebAccessFetchCrawlSearchSource {
  type: 'search';
  searchQuery: string;
}
export type WebAccessFetchCrawlSource = WebAccessFetchCrawlUrlSource | WebAccessFetchCrawlSearchSource;
export interface WebAccessFetchReadRequest {
  url: string;
  maxChars?: number | undefined;
}
export interface WebAccessFetchCrawlUrlRequest {
  source: WebAccessFetchCrawlUrlSource;
  query: string;
  topK?: number | undefined;
  maxPages?: number | undefined;
  maxChars?: number | undefined;
}
export interface WebAccessFetchCrawlSearchRequest {
  source: WebAccessFetchCrawlSearchSource;
  query: string;
  topK?: number | undefined;
  maxPages?: number | undefined;
  maxChars?: number | undefined;
...
export interface WebAccessFetchSitemapRequest {
  url: string;
  siteMap: true;
...
function parseSourceIds(value: unknown): string[] | undefined {
  ...
  const out: string[] = [];
  for (const entry of value) {
    out.push(cleanSourceIdEntry(entry));
  }
  return out;
}

function cleanSourceIdEntry(entry: unknown): string {
  if (typeof entry !== 'string' || entry.trim().length === 0) {
...
function parseCrawlBounds(raw: Record<string, unknown>): { topK?: number | undefined; maxPages?: number | undefined; maxChars?: number | undefined } {
  const out: { topK?: number | undefined; maxPages?: number | undefined; maxChars?: number | undefined } = {};
  if (raw.topK !== undefined) {
    if (!Number.isInteger(raw.topK) || (raw.topK as number) < 1 || (raw.topK as number) > WEB_ACCESS_FETCH_TOP_K_MAX) {
      throw new WebAccessContractError(`topK must be an integer in [1, ${WEB_ACCESS_FETCH_TOP_K_MAX}]`);
    }
    out.topK = raw.topK as number;
  }
  if (raw.maxPages !== undefined) {
    if (!Number.isInteger(raw.maxPages) || (raw.maxPages as number) < 1 || (raw.maxPages as number) > WEB_ACCESS_FETCH_MAX_PAGES_MAX) {
      throw new WebAccessContractError(`maxPages must be an integer in [1, ${WEB_ACCESS_FETCH_MAX_PAGES_MAX}]`);
    }
    out.maxPages = raw.maxPages as number;
  }
  if (raw.maxChars !== undefined) {
    if (!Number.isInteger(raw.maxChars) || (raw.maxChars as number) < 1 || (raw.maxChars as number) > WEB_ACCESS_FETCH_MAX_CHARS_MAX) {
      throw new WebAccessContractError(`maxChars must be an integer in [1, ${WEB_ACCESS_FETCH_MAX_CHARS_MAX}]`);
    }
    out.maxChars = raw.maxChars as number;
  }
  return out;
}

function parseCrawlSource(raw: Record<string, unknown>): WebAccessFetchCrawlSource {
  const source = raw.source;
  if (typeof source !== 'object' || source === null || Array.isArray(source)) {
    throw new WebAccessContractError('crawl requires source {type:url url followLinks?} or {type:search searchQuery}');
  }
  const typed = (source as Record<string, unknown>).type;
  if (typed === 'url') {
    const url = cleanUrl((source as Record<string, unknown>).url);
    if (url === undefined) throw new WebAccessContractError('source url must be a non-empty string');
    const followLinks = (source as Record<string, unknown>).followLinks;
    if (followLinks !== undefined && followLinks !== true) {
      throw new WebAccessContractError('source followLinks must be true when provided');
    }
    return { type: 'url', url, ...(followLinks === true ? { followLinks: true as const } : {}) };
  }
  if (typed === 'search') {
    const searchQuery = (source as Record<string, unknown>).searchQuery;
    if (typeof searchQuery !== 'string' || searchQuery.trim().length === 0) {
      throw new WebAccessContractError('source searchQuery must be a non-empty string');
    }
    return { type: 'search', searchQuery: searchQuery.trim() };
  }
  throw new WebAccessContractError('source type must be one of: url, search');
}

function parseCrawlFetch(raw: Record<string, unknown>): WebAccessFetchCrawlUrlRequest | WebAccessFetchCrawlSearchRequest {
  const source = parseCrawlSource(raw);
  const query = cleanQuery(raw.query);
  if (query === undefined) throw new WebAccessContractError('crawl requires query (passage selector)');
  rejectPresentFields(raw, CRAWL_REJECTED_TOP_LEVEL, 'crawl accepts only source/query/topK/maxPages/maxChars');
  const bounds = parseCrawlBounds(raw);
  if (source.type === 'url') return { source, query, ...bounds };
  return { source, query, ...bounds };
}

const CRAWL_REJECTED_TOP_LEVEL = ['url', 'urls', 'searchQuery', 'followLinks', 'siteMap', 'sitemap', 'action', 'responseId', 'sourceIds', 'claims', 'offset', 'limit', 'findText'] as const;

function parseReadFetch(raw: Record<string, unknown>): WebAccessFetchReadRequest {
  const url = cleanUrl(raw.url);
  if (url === undefined) throw new WebAccessContractError('read requires url');
  rejectPresentFields(raw, ['source', 'urls', 'searchQuery', 'query', 'topK', 'maxPages', 'followLinks', 'siteMap', 'sitemap', 'action', 'responseId', 'sourceIds', 'claims', 'offset', 'limit', 'findText'], 'read accepts only url/maxChars');
  if (raw.maxChars !== undefined) {
    if (!Number.isInteger(raw.maxChars) || (raw.maxChars as number) < 1 || (raw.maxChars as number) > WEB_ACCESS_FETCH_MAX_CHARS_MAX) {
      throw new WebAccessContractError(`maxChars must be an integer in [1, ${WEB_ACCESS_FETCH_MAX_CHARS_MAX}]`);
    }
    return { url, maxChars: raw.maxChars as number };
  }
  return { url };
}
...
function parseUrlsFetch(raw: Record<string, unknown>): WebAccessFetchMultiReadRequest | WebAccessFetchMultiCrawlRequest {
  ...
  if (raw.query === undefined) {
    ...
    if (raw.maxChars !== undefined) {
      ...
      return { urls, maxChars: raw.maxChars as number };
    }
    return { urls };
  }
  const query = cleanQuery(raw.query);
  if (query === undefined) throw new WebAccessContractError('query must be a non-empty string when provided');
  return { urls, query, ...parseCrawlBounds(raw) };
}

function parseSitemapFetch(raw: Record<string, unknown>): WebAccessFetchSitemapRequest {
  if (raw.siteMap !== true) throw new WebAccessContractError('sitemap requires siteMap:true');
  const url = cleanUrl(raw.url);
  if (url === undefined) throw new WebAccessContractError('sitemap requires url');
  rejectPresentFields(raw, ['source', 'urls', 'searchQuery', 'topK', 'maxChars', 'followLinks', 'sitemap', 'action', 'responseId', 'sourceIds', 'claims', 'offset', 'limit', 'findText'], 'sitemap accepts only url/siteMap/query/maxPages');
  const out: WebAccessFetchSitemapRequest = { url, siteMap: true };
  if (raw.query !== undefined) {
    const query = cleanQuery(raw.query);
    if (query === undefined) throw new WebAccessContractError('query must be a non-empty string when provided');
    out.query = query;
...
function parseResponseId(raw: Record<string, unknown>, action: 'retrieve' | 'source_check'): string {
  if (typeof raw.responseId !== 'string' || raw.responseId.trim().length === 0) throw new WebAccessContractError(`${action} requires responseId`);
...
function parseClaims(raw: Record<string, unknown>): string[] {
  ...
  for (const entry of raw.claims) {
    claims.push(cleanClaimEntry(entry));
  }
  return claims;
}

function rejectPresentFields(raw: Record<string, unknown>, fields: readonly string[], message: string): void {
  for (const field of fields) {
    if (raw[field] !== undefined) throw new WebAccessContractError(message);
  }
}
...
function parseNormalFetch(raw: Record<string, unknown>): WebAccessFetchReadRequest | WebAccessFetchCrawlUrlRequest | WebAccessFetchCrawlSearchRequest | WebAccessFetchMultiReadRequest | WebAccessFetchMultiCrawlRequest | WebAccessFetchSitemapRequest {
  ...
  if (raw.url !== undefined) {
    ...
    return parseReadFetch(raw);
  }
  throw new WebAccessContractError('fetch requires exactly one of: url, urls, source, or url+siteMap:true');
}

function parseFindTextValue(value: unknown): string | undefined {
  if (value === undefined) return undefined;
...
function parseSliceOffsetValue(value: unknown): number | undefined {
  if (value === undefined) return undefined;
...
function parseSliceLimitValue(value: unknown): number | undefined {
  if (value === undefined) return undefined;
  if (!Number.isInteger(value) || (value as number) < 1 || (value as number) > WEB_ACCESS_RETRIEVAL_MAX_CHARS) {
...
function parseRetrieveFetch(raw: Record<string, unknown>): WebAccessFetchRetrieveRequest {
  const responseId = parseResponseId(raw, 'retrieve');
  ...
  const findText = parseFindTextValue(raw.findText);
  if (findText !== undefined) out.findText = findText;
  const offset = parseSliceOffsetValue(raw.offset);
  if (offset !== undefined) out.offset = offset;
  const limit = parseSliceLimitValue(raw.limit);
  if (limit !== undefined) out.limit = limit;
  rejectPresentFields(raw, ['url', 'urls', 'source', 'searchQuery', 'query'], 'retrieve accepts only responseId/sourceIds/offset/limit/findText');
  rejectPresentFields(raw, ['topK', 'maxPages', 'maxChars', 'followLinks', 'sitemap', 'siteMap', 'claims'], 'retrieve accepts only responseId/sourceIds/offset/limit/findText');
  return out;
}

function parseSourceCheckFetch(raw: Record<string, unknown>): WebAccessFetchSourceCheckRequest {
  const responseId = parseResponseId(raw, 'source_check');
  const claims = parseClaims(raw);
  const out: WebAccessFetchSourceCheckRequest = { action: 'source_check', responseId, claims };
  const sourceIds = parseSourceIds(raw.sourceIds);
  if (sourceIds !== undefined) out.sourceIds = sourceIds;
  rejectPresentFields(raw, ['url', 'urls', 'source', 'searchQuery', 'query', 'offset', 'limit', 'findText', 'topK', 'maxPages', 'maxChars', 'followLinks', 'sitemap', 'siteMap'], 'source_check accepts only responseId/claims/sourceIds');
  return out;
}
...
export function normalizeWebAccessContentSlice(raw: WebAccessContentSlice): WebAccessContentSlice {
  ...
  if (findText !== undefined) {
    const out: WebAccessContentSlice = { findText };
