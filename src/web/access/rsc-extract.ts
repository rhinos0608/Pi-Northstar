/**
 * RSC Content Extractor (dependency-free, no DOM).
 *
...
export const RSC_MIN_USEFUL_CONTENT = 500;
export const RSC_MIN_EXTRACTED_CONTENT = 100;
...
export function extractRSCContent(html: string): RSCExtractResult | null {
  ...
  function extractTable(tableNode: unknown[]): string {
    ...
    function walkTable(node: unknown, isHeader = false): void {
      ...
      if (typeof node === "string") {
        const refMatch = node.match(/^\$L([0-9a-f]+)$/i);
        const tableRefId = refMatch?.[1];
        if (tableRefId !== undefined && !visitedRefs.has(tableRefId)) {
          visitedRefs.add(tableRefId);
          const refNode = getParsedChunk(tableRefId);
          if (refNode) walkTable(refNode, isHeader);
          visitedRefs.delete(tableRefId);
        }
        return;
      }
    ...
    function walkCells(node: unknown, cells: string[]): void {
      ...
      if (typeof node === "string") {
        const refMatch = node.match(/^\$L([0-9a-f]+)$/i);
        const cellRefId = refMatch?.[1];
        if (cellRefId !== undefined && !visitedRefs.has(cellRefId)) {
          visitedRefs.add(cellRefId);
          const refNode = getParsedChunk(cellRefId);
          if (refNode) walkCells(refNode, cells);
          visitedRefs.delete(cellRefId);
        }
        return;
      }
      ...
      if (node[0] === "$" && (node[1] === "td" || node[1] === "th")) {
        ...
        const text = extractNode(cellProps.children, { inTable: true, inCode: false })
          .trim()
          .replace(/\n/g, " ")
          .replace(/\\/g, "\\\\")  // Escape backslashes firs
  ...
  if (mainChunk) {
    ...
    if (content.trim().length > RSC_MIN_EXTRACTED_CONTENT) {
  ...
  return content.length > RSC_MIN_EXTRACTED_CONTENT ? { title, content } : null;
}
