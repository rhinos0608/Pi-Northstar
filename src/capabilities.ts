...
export const CHANNEL_CAPABILITIES: readonly ChannelCapability[] = [
  ...
  },
  {
    ...
    publicTool: 'github',
    availability: 'available',
    description: 'GitHub repositories, files, trees, search, trending, issues, pulls, releases, commits',
    tier: 0,
    domains: ['github.com'],
    actions: [
      act('repo'), act('file'), act('tree'),
      act('search'), act('search_repos'), act('trending'), act('issues'),
      act('pulls'), act('releases'), act('commits'),
    ],
    backends: [{
      id: 'github-api',
      mode: 'native',
      quality: 'full',
      actions: ['repo', 'file', 'tree', 'search', 'search_repos', 'trending', 'issues', 'pulls', 'releases', 'commits'],
