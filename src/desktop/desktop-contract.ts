...
export const MAX_SELECTOR_TEXT_LENGTH=200; export const MAX_ID_LENGTH=200; export const MAX_COORD_ABS=100000;
// Validation errors name fields, never user-controlled values. Window/state ids
// and predicates can carry sensitive UI text just as typed text/keys can.
function invalidParam(key: string): Error { return new Error(`INVALID_REQUEST: invalid ${key}`); }
...
export function validateDesktopRequest(raw: Record<string, unknown>): DesktopRequest {
  ...
  for (const key of ['windowId','stateId','text','key'] as const) if (raw[key]!==undefined) { const limit = key==='text' ? MAX_TEXT_LENGTH : MAX_ID_LENGTH; if(typeof raw[key]!=='string'||(raw[key] as string).length>limit) throw invalidParam(key); req[key]=raw[key] as never; }
  ...
  if (raw.predicate!==undefined) { if(typeof raw.predicate!=='object'||raw.predicate===null||Array.isArray(raw.predicate)) throw new Error('INVALID_REQUEST: predicate must be object'); const p=raw.predicate as Record<string,unknown>; for (const key of Object.keys(p)) if(key!=='text'&&key!=='role') throw new Error(`INVALID_REQUEST: unknown predicate field ${key}`); for (const key of ['text','role'] as const) if (p[key]!==undefined && (typeof p[key]!=='string'||(p[key] as string).length>MAX_SELECTOR_TEXT_LENGTH)) throw invalidParam(`predicate.${key}`); req.predicate={...(typeof p.text==='string'?{text:p.text}:{}),...(typeof p.role==='string'?{role:p.role}:{})}; }
