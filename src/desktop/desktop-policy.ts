import { DESKTOP_ACTIONS, DESKTOP_ACTION_CONTRACT, type DesktopAction, isMutation, validateDesktopRequest, type DesktopRequest } from './desktop-contract.js';
...
export function validatePolicy(raw:Record<string,unknown>, env:Record<string,string|undefined>=process.env):DesktopRequest { if(!desktopEnabled(env)) throw new Error('DESKTOP_DISABLED: set PI_SEARCH_DESKTOP_AUTOMATION=1 to enable'); const request=validateDesktopRequest(raw); assertAllowedAction(request.action); if(isDeniedUpstreamTool(request.action)) throw new Error(`ACTION_DENIED: ${request.action} is denied`); if((request.action==='observe_window'||request.action==='wait')&&(!request.pid||!request.windowId)) throw new Error('INVALID_REQUEST: pid and windowId required'); if(request.includeScreenshot!==undefined&&request.action!=='observe_window') throw new Error('INVALID_REQUEST: includeScreenshot only valid for observe_window'); if(isMutation(request.action)&&request.stateId===undefined) throw new Error('STALE_OBSERVATION: mutation requires stateId'); const allowed=new Set<string>(DESKTOP_ACTION_CONTRACT[request.action].allowed); for (const key of Object.keys(request)) { if(key==='action') continue; if(!allowed.has(key)) throw new Error(`INVALID_REQUEST: field ${key} not valid for action ${request.action}`); } if(request.action==='type_text'&&(request.text===undefined||request.text.length===0)) throw new Error('INVALID_REQUEST: text required for type_text'); if(request.action==='press_key'&&(request.key===undefined||request.key.length===0)) throw new Error('INVALID_REQUEST: key required for press_key'); return request; }
