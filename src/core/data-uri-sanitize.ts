...
// Atlas fetch-output hygiene: replaces inline RFC 2397 data: URIs in
// extracted text with explicit bounded omission markers so base64 payloads
// never reach model-visible tool results or the fetch cache.
// Never redacts thumbnails/frames: Atlas fetch output carries no such
// fields, so sanitization applies to `content` text only (never inside
// `details` envelopes that carry citations).

export const MAX_DATA_URI_HEADER_CHARS = 1024;
export const MAX_MIME_CHARS = 127;
export const MAX_MARKER_SOURCE_CHARS = 180;
...
/** Scan one enclosed candidate. Balanced inner parentheses are consumed so invalid payload suffixes cannot escape. Unterminated enclosures report closed: false. */
function scanEnclosedCandidate(text: string, start: number, enclosure: NonNullable<DataUriEnclosure>): { end: number; comma: number; closed: boolean } {
	let comma = -1;
	...
	for (; index < text.length; index++) {
		...
		if (enclosure.kind === "quote") {
			if (ch === enclosure.close) return { end: index, comma, closed: true };
		} else if (enclosure.kind === "parentheses") {
			if (ch === "(") {
				depth++;
			} else if (ch === ")") {
				depth--;
				if (depth === 0) return { end: index, comma, closed: true };
			}
		} else if (enclosure.kind === "angle") {
			if (ch === ">") return { end: index, comma, closed: true };
		}
		if (comma < 0 && ch === ",") comma = index;
	}

	return { end: index, comma, closed: false };
}

function scanBareCandidate(text: string, start: number): { end: number; comma: number } {
	let comma = -1;
	let index = start + 5;

	for (; index < text.length; index++) {
		const ch = text[index]!;
		if (isBareDataUriTerminator(ch)) break;
		if (comma < 0 && ch === ",") comma = index;
	}

	return { end: index, comma };
}
...
function scanDataUriCandidate(text: string, start: number): ScannedDataUriCandidate {
	...
	if (enclosure !== null) {
		const enclosed = scanEnclosedCandidate(text, start, enclosure);
		if (enclosed.closed) return { end: enclosed.end, comma: enclosed.comma, enclosure };
	}
	const bare = scanBareCandidate(text, start);
	return { end: bare.end, comma: bare.comma, enclosure: null };
}
...
function buildDataUriMarker(omission: DataUriOmission): string {
	...
	return `[pi-northstar inline data URI omitted; ordinal=${omission.ordinal}; source=${omission.sourcePath}; mime=${omission.mimeType}; encoding=${omission.encoding}; encodedBytes=${omission.encodedBytes}; decodedBytes=${decoded}${decodeError}; sha256=${omission.sha256}; digestBasis=${omission.digestBasis}; retrieval=not-retained]`;
}
